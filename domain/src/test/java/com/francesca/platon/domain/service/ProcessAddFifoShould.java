package com.francesca.platon.domain.service;

import com.francesca.platon.domain.model.Process;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.mockito.InjectMocks;
import org.mockito.Mock;
import org.mockito.junit.jupiter.MockitoExtension;

import java.util.concurrent.ArrayBlockingQueue;

import static org.mockito.Mockito.*;

@ExtendWith(MockitoExtension.class)
class ProcessAddFifoShould {

    private static final int CAPACITY = 2;

    @Mock
    private final ArrayBlockingQueue<Process> queue = new ArrayBlockingQueue<>(CAPACITY);

    @InjectMocks
    private ProcessAddFifo processAddFifo;

    @BeforeEach
    public void init() {
        queue.clear();
    }

    @Test
    void add_a_process_on_the_queue_if_there_is_remaining_space() {
        var process = mock(Process.class);
        when(queue.remainingCapacity()).thenReturn(1);

        processAddFifo.addProcess(queue, process);

        verify(queue).add(process);
        verify(process).run();
    }

    @Test
    void do_not_add_a_process_on_the_queue_if_there_is_no_space() {
        var process = mock(Process.class);
        when(queue.remainingCapacity()).thenReturn(0);

        processAddFifo.addProcess(queue, process);

        verify(queue).add(process);
        verify(process).run();
    }

    @Test
    void remove_oldest_process_if_the_queue_is_full() {
        var process1 = mock(Process.class);
        var process2 = mock(Process.class);
        var process3 = mock(Process.class);
        when(queue.poll()).thenReturn(process1);

        queue.add(process1);
        queue.add(process2);

        processAddFifo.addProcess(queue, process3);

        verify(queue).add(process1);
        verify(queue).add(process2);
        verify(queue).poll();
        verify(process1).kill();
        verify(queue).add(process3);
    }
}